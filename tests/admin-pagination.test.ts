...
describe('admin list pagination', () => {
  it('returns exact filtered user totals and clamps out-of-range pages', async () => {
    await seedUser(db, { id: '8000' })
    const now = Date.now()
    await db.batch(Array.from({ length: 7 }, (_, index) => db.prepare(
      `INSERT INTO user
    ...
    const first = await searchUsersPage(db, { role: 'user', page: 1, pageSize: 3 })
    expect(first).toMatchObject({ total: 5, page: 1, pageSize: 3, totalPages: 2 })
    expect(first.items).toHaveLength(3)

    const last = await searchUsersPage(db, { role: 'user', page: 99, pageSize: 3 })
  ...
  it('paginates all DNS records with a stable ordering and exact total', async () => {
    const userId = await seedUser(db, { id: '8200' })
    const now = Date.now()
    await db.batch(Array.from({ length: 7 }, (_, index) => db.prepare(
    ...
    const second = await listAllRecordsPage(db, { page: 2, pageSize: 3 })
    ...
    const last = await listAllRecordsPage(db, { page: 9, pageSize: 3 })
